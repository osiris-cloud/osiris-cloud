...
def namespaced(view_func):
    ...
    def _wrapped_view(request, *args, **kwargs):
        ...
        if not ns:
            return render(request, "404_app.html", status=404)  # Render 404 if namespace is not found

        role = ns.get_role(request.user)

        if role is None:
            return render(request, "404_app.html", status=404)  # If the user has no role, render 404
